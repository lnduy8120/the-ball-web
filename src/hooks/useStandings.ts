...
export const useStandings = (leagueId: string = 'PL', initialData?: StandingRow[]) => {
  const [filter, setFilter] = useState('overall'); // overall, home, away

  const { data = [], isLoading: loading } = useQuery({
    queryKey: ['standings', leagueId],
    queryFn: () => teamService.getStandings(leagueId),
    staleTime: 1000 * 60 * 60, // 1 hour (standings don't change often)
    initialData: leagueId === 'PL' ? initialData : undefined,
  });
